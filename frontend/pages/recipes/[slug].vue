          ...
          <div class="mb-8">
            ...
            <ul class="list-disc list-inside">
              <li v-for="(item, index) in recipe.ingredients" :key="index" class="capitalize">{{ item.name }} ({{ item.qty }} {{ item.unit }}) <span v-if="item.type == 'protein'">- <span class="font-medium">${{ item.price }}</span></span> 
                <button v-if="item.type == 'protein'" class="w-full	md:w-auto mb-2 md:mb-auto bg-blue-600 hover:bg-blue-700 text-white font-bold py-1.5 px-1.5 ml-1.5 rounded text-xs">Add to Basket</button></li>
            </ul>
          </div>
